...
import {
  AgentKnowledgeChunk,
  AgentKnowledgeProbeResult,
  KnowledgeBase,
  getAllAgentKnowledgeChunks,
  probeAgentKnowledgeChunks,
...
type ChunkPoint = AgentKnowledgeChunk & {
  index: number;
  mapX: number;
  mapY: number;
  sourceId: string;
...
function buildBasePoints(
  ...
  const sourceIndexMap = new Map(
    sourceOptions.map((source, index) => [source.id, index]),
  );

  return chunks.map((chunk, index) => {
    const chunkSignature = signature(chunk.content);
    const projection = projectTo2D(chunkSignature, chunk.tokenCount);
    const sourceLabel = chunk.fileName || chunk.sourceType || "Unknown";
    const sourceIndex = sourceIndexMap.get(chunk.knowledgeBaseId) ?? 0;
...
function buildAnalysis(
  ...
  const basePoints = buildBasePoints(chunks, sourceOptions);
...
export default function AgentKnowledgebasesInspector({
  agentId,
  knowledgeBases,
  children,
}: Readonly<{
  ...
  const [probeResults, setProbeResults] = useState<AgentKnowledgeProbeResult[]>(
    [],
  );
  const [selectedChunkId, setSelectedChunkId] = useState<string | null>(null);
  const [chunks, setChunks] = useState<AgentKnowledgeChunk[] | null>(null);
  const [isLoadingChunks, startLoadChunks] = useTransition();
  const [isProbing, startProbe] = useTransition();

  const sourceOptions = useMemo(() => {
    const base: SourceOption[] = knowledgeBases.map((source) => ({
      id: source.id,
      label: source.fileName || source.sourceType || "Untitled",
  ...
  const analysis = useMemo(
    () => buildAnalysis(chunks ?? [], sourceOptions),
    [chunks, sourceOptions],
  );
  ...
  const filteredPoints = useMemo(() => {
    ...
    return points.filter((point) => point.sourceId === selectedSourceId);
  }, [analysis.points, selectedSourceId]);
  ...
  const filteredChunks = useMemo(() => {
    ...
    return filteredPoints.filter((chunk) =>
      `${chunk.content} ${chunk.tokenCount} ${chunk.sourceLabel}`
        .toLowerCase()
        .includes(keyword),
    );
  ...
  const selectedChunk = useMemo(
    () => filteredPoints.find((chunk) => chunk.id === selectedChunkId) ?? null,
    [filteredPoints, selectedChunkId],
  );

  const localSimilarChunks = useMemo(
    () => getLocalSimilarChunks(filteredPoints, selectedChunkId),
    [filteredPoints, selectedChunkId],
  );

  const health = useMemo(
    () => getHealthSummary(filteredPoints, analysis.duplicates),
    [analysis.duplicates, filteredPoints],
  );

  const legendSources = useMemo(() => {
    const counts = new Map<string, number>();
    for (const point of analysis.points) {
      counts.set(point.sourceId, (counts.get(point.sourceId) ?? 0) + 1);
    }
  ...
  }, [analysis.points, sourceOptions]);

  const mapPoints = useMemo(
    () => filteredPoints.slice(0, 360),
    [filteredPoints],
  );
  ...
  const loadChunks = () => {
    ...
    startLoadChunks(async () => {
      try {
        const result = await getAllAgentKnowledgeChunks(agentId);
  ...
  const runQueryProbe = () => {
    ...
    startProbe(async () => {
      try {
        const result = await probeAgentKnowledgeChunks(cleaned, agentId, 12);
  ...
  const filteredProbeResults = useMemo(() => {
    if (selectedSourceId === "all") {
      return probeResults;
    }

    return probeResults.filter(
      (item) => item.knowledgeBaseId === selectedSourceId,
    );
  }, [probeResults, selectedSourceId]);
    ...
    return (
      <div className="rounded-lg border bg-muted/10 p-3">
        <div className="flex items-center justify-between pb-2">
          <p className="text-sm font-medium">UMAP projection</p>
          <div className="flex items-center gap-2">
            <Badge variant="secondary">{filteredPoints.length} chunks</Badge>
        ...
        >
          {mapPoints.map((point) => {
            ...
            const colorClass =
              SOURCE_COLORS[point.sourceIndex % SOURCE_COLORS.length];
            ...
            return (
              <g key={point.id}>
                ...
                >
                  <title>
                    {`Chunk ${point.index + 1} | ${point.tokenCount} tokens | ${point.sourceLabel}`}
                  </title>
                </circle>
              </g>
            );
        ...
        <div className="flex flex-wrap gap-3 pt-3 text-xs text-muted-foreground">
          {legendSources.map((source) => (
            <div key={source.id} className="flex items-center gap-2">
              <span className={cn("size-2 rounded-full", source.colorClass)} />
              <span>
                {truncateCharacters(source.label, 20)} ({source.count})
              </span>
            </div>
          ))}
        </div>
    ...
    <div className="flex flex-col gap-3">
      ...
      <ScrollArea className="h-[390px] rounded-lg border p-3">
        <div className="flex flex-col gap-2">
          {filteredProbeResults.length === 0 && (
            <p className="text-sm text-muted-foreground">
              Run a probe to see ranked chunks and similarity scores.
            </p>
          )}

          {filteredProbeResults.map((result, index) => {
            ...
            const sourceLabel =
              result.fileName || result.sourceType || "Unknown";
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {truncateCharacters(sourceLabel, 24)}
                </p>
              ...
              >
                ...
                <TableCell>
                  <Badge variant="outline">
                    {truncateCharacters(chunk.sourceLabel, 16)}
                  </Badge>
                </TableCell>
                <TableCell>
  ...
  const renderHealthView = () => (
    <div className="grid md:grid-cols-2 gap-3">
      ...
      <div className="rounded-lg border p-3 bg-muted/10">
        ...
        <p className="text-xl font-semibold">{sourceOptions.length}</p>
        <p className="text-xs text-muted-foreground mt-1">
          sources currently attached to this agent
        </p>
      </div>
    </div>
  );
  ...
  const renderDetailPanel = () => {
    if (!selectedChunk) {
      return (
        <div className="rounded-lg border p-3 bg-muted/10 min-h-[420px]">
          <p className="text-sm text-muted-foreground">
    ...
    return (
      <div className="rounded-lg border p-3 bg-muted/10 min-h-[420px] flex flex-col gap-3">
        <div className="flex items-start justify-between gap-2">
          <div>
            <p className="text-sm font-medium">Chunk inspector</p>
            <p className="text-xs text-muted-foreground">
              {getChunkLabel(selectedChunk)}
            </p>
          </div>
          <Badge variant="outline">
            {truncateCharacters(selectedChunk.sourceLabel, 14)}
          </Badge>
        </div>
        ...
        <div className="rounded border p-2">
          ...
          <p className="text-sm">{selectedChunk.sourceLabel}</p>
          <p className="text-xs text-muted-foreground mt-1">
            Coordinates ({selectedChunk.mapX.toFixed(2)},{" "}
            {selectedChunk.mapY.toFixed(2)})
          </p>
        </div>
        ...
        <div className="rounded border p-2">
          <p className="text-xs text-muted-foreground">Find similar chunks</p>
              ...
              <SelectContent>
                ...
                {sourceOptions.map((source) => (
                  <SelectItem key={source.id} value={source.id}>
                    {truncateCharacters(source.label, 28)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
                ...
                >
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      placeholder="Search by content, token, source"
                      className="max-w-sm"
                    />
                    <Badge variant="secondary">
                      {filteredChunks.length} rows
                    </Badge>
                  </div>
                  {renderBrowserView()}
                </TabsContent>
