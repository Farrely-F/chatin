...
- [x] **ACTION**: Verify clean working directory with `git status` (currently not clean due in-progress migration edits)
...
- [x] **ACTION**: Run `pnpm add ai@latest` (completed via npm fallback)
- [x] **ACTION**: Verify version in package.json shows ^5.0.0 or higher
- [x] **INFO**: New version installed: ^5.0.154
...
- [x] **ACTION**: Run `pnpm install` to ensure lock file is updated (completed via npm install fallback)
...
- [x] **ACTION**: Run `pnpm install` (completed via npm install fallback)
...
- [x] **ACTION**: Create a plan for addressing each FIXME in Phase 4 (remove maxSteps from two useChat hooks, migrate tool part rendering from toolInvocation to v5 tool-\* parts in two chat UIs)
...
- [x] **INFO**: Files updated: src/components/ui/chat.tsx, src/features/public-chat/chat.tsx
...
- [x] **Response methods**: `toDataStreamResponse` → `toUIMessageStreamResponse`
...
- [x] **useChat**: Use `DefaultChatTransport` wrapper
- [x] **Methods**: `append` → `sendMessage`, `reload` → `regenerate`
- [x] **Props**: `initialMessages` → `messages`, `isLoading` → `status`
- [x] **Input management**: Now manual (use `useState`)
...
- [x] **Message IDs**: Move `experimental_generateMessageId` to `toUIMessageStreamResponse`
- [x] **Multi-step**: Replace `maxSteps` with `stopWhen`
...
- [x] **Content assignment**: Can't do `message.content = "..."`, use `message.parts` instead
- [x] **Empty checks**: Check `parts`, not `content`
- [x] **Tool states**: All updated to new names
...
- [x] `pnpm lint` passes (if applicable) (build completed with non-blocking lint warnings)
